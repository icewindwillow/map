...
test('homepage references new resource URLs rather than the old cached URLs', async () => {
  assert.match(html, /\.\/assets\/style\.v0\.2\.1\.css/);
  assert.match(html, /\.\/assets\/app\.v0\.2\.1\.js/);
...
test('the actual deployed app imports the matching versioned helper', async () => {
  const app = await readFile(new URL('assets/app.v0.2.1.js', root), 'utf8');
  assert.match(app, /from '\.\/geo\.v0\.2\.1\.js'/);
  assert.doesNotMatch(app, /from '\.\/geo\.js'/);
  assert.ok((await stat(new URL('assets/geo.v0.2.1.js', root))).isFile());
