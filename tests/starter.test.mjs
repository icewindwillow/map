...
import {UK_BOUNDS,project,unproject,validateMemories,filterMemories,clusterPoints} from '../public/assets/geo.js';
...
test('no personal stories are fabricated',async()=>{const d=JSON.parse(await read('data/memories.json'));assert.equal(d.schemaVersion,1);assert.deepEqual(d.memories,[]);});
...
test('runtime renders user text without innerHTML',async()=>{assert.doesNotMatch(await read('assets/app.js'),/\.innerHTML\s*=/);});
