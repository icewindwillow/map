import {CATEGORIES,categoryOf,categoryIcon,ratingTone} from './place-style.v0.6.js';
...
function select(id,newPlace=null){
  ...
  const category=current.category||'其他';for(const o of [...$('category').options])if(o.dataset.legacy)o.remove();if(!CATEGORIES.includes(category)){const option=new Option(category,category);option.dataset.legacy='true';$('category').append(option);}
  for(const [key,field]of Object.entries(fields))$(field).value=current[key]|| (key==='region'?'england':key==='category'?'其他':'');categoryUI();
...
async function load(){
  ...
  try{
    ...
    const [seed,saved,cloud]=await Promise.all([api('/data/fountains-abbey.v0.3.json'),api('/data/memories.json?v=0.5.4'),api('/author/api/places')]);
...
function createPlace(){select('place-'+crypto.randomUUID(),{place:'',title:'',region:'england',coordinates:null,photos:[],review:{author:$('review-author').value||'',rating:null,comment:''}});dirty=true;$('location-query').focus();message('请先搜索地名并选择候选地点，位置会自动填写。');}
...
$('place-form').addEventListener('input',()=>{dirty=true;ratingUI();categoryUI();drawPins();});
