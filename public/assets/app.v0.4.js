...
import {UK_BOUNDS,REGIONS,project,unproject,validateMemories,filterMemories,clusterPoints,featurePath,validRating,starFills,photosOf,mergeMemories,publishableMemories} from './geo.v0.4.js';
...
function expandCluster(items){
  ...
  if(state.online){const b=new GeoBounds();items.forEach(m=>b.extend(m.coordinates));streetMap.fitBounds(b,{padding:95,maxZoom:15,duration:reduced?0:650});}
...
async function refreshCloudReviews(){
  ...
  try{
    ...
    state.real=state.real.map(m=>entries.has(m.id)?{...m,review:entries.get(m.id).review}:m);
    const selected=state.selected?.id;
    refreshCollection();
    if(selected){const m=state.real.find(x=>x.id===selected);if(m){state.selected=m;$('review-author').textContent=m.review.author;renderRating($('story-rating'),m.review.rating);$('review-comment').textContent=m.review.comment;$('review-comment').hidden=!m.review.comment;$('review-empty').hidden=!!m.review.comment;}}
    $('cloud-read-status').textContent='评价 · 云端已发布版本';$('cloud-read-status').title='草稿不会出现在公开地图中。';
