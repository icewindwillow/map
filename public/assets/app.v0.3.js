import {UK_BOUNDS,REGIONS,project,unproject,validateMemories,filterMemories,clusterPoints,featurePath,validRating,starFills,photosOf,mergeMemories,publishableMemories} from './geo.v0.3.js';
...
function refreshCollection(){
  ...
  state.filtered.forEach((m,i)=>{
    ...
    if(photos.length){const thumb=document.createElement('img');thumb.className='row-photo';thumb.src=photoSrc(photos[0].thumbnail||photos[0].src);thumb.alt='';thumb.loading='lazy';row.append(thumb);}else row.append(text('span','row-number',String(i+1).padStart(2,'0')));
    const body=text('span','row-body');const title=text('span','row-title');title.append(text('strong','',m.place||'地点待补'));if(m.placeEn)title.append(text('small','',m.placeEn));body.append(title,text('span','row-sub',[REGIONS[m.region],m.category,m.demo?'演示':m.date,!m.coordinates?'定位待补':''].filter(Boolean).join(' · ')));
...
function openStory(m,trigger=null){
  ...
  $('story-region').textContent=[m.locationLabel||REGIONS[m.region],m.category,m.date].filter(Boolean).join(' · ');
  $('story-place').replaceChildren(document.createTextNode(m.place||'地点待补'));if(m.placeEn)$('story-place').append(text('small','',m.placeEn));
  $('story-writing').hidden=!m.description;$('story-title').hidden=!m.title||m.title===m.place;
...
async function enterStreet(focus=null){
  ...
  if(state.online){if(focus)streetMap.flyTo({center:focus,zoom:13,padding:{right:0},duration:reduced?0:800});return;}
  ...
  try{
    ...
    if(focus)thisMap.flyTo({center:focus,zoom:13,padding:{right:0},duration:reduced?0:900});
  }catch(error){
    if(token!==streetToken)return;
...
async function loadStories(first=false){
  ...
  const failures=[];
  for(const [key,path] of [['firstMemory','./data/fountains-abbey.v0.3.json'],['memories','./data/memories.json?v=0.3']]){
  ...
  state.real=mergeMemories(state.seed||[],state.saved||[]);
  if(first)$('show-demos').checked=state.real.length===0;
...
document.addEventListener('keydown',e=>{
  if($('photo-viewer').open||$('author-dialog').open||$('about-dialog').open)return;
...
const draftKey='uk-memory-map.author-reviews.v1';
let editorItems=[];
let editorId=null;
...
function renderRating(target,value,{compact=false}={}){
  target.replaceChildren();target.classList.toggle('compact-rating',compact);
...
/* Static author desk: edit locally, then export and deploy. Never a pretend authenticated backend. */
function authorStatus(message,error=false){$('author-status').textContent=message;$('author-status').classList.toggle('is-error',error);}
function editorMemory(){return editorItems.find(m=>m.id===editorId);}
function captureReview(){
  const m=editorMemory();if(!m)return;
  const rating=$('author-unrated').checked?null:Number($('author-rating').value);
  if(rating!==null&&!validRating(rating))throw new Error('评分只能为 0～5 星，每次半星。');
  m.review={...(m.review||{}),author:$('author-name').value,rating,comment:$('author-comment').value};
}
function previewAuthor(){
  const value=$('author-unrated').checked?null:Number($('author-rating').value);
  const m=editorMemory();
  $('author-rating').disabled=$('author-unrated').checked;
  $('rating-minus').disabled=$('author-unrated').checked||value===0;
  $('rating-plus').disabled=$('author-unrated').checked||value===5;
  $('author-rating-value').textContent=value===null?'尚未评分':`${value.toFixed(1)} / 5`;
  $('author-rating').setAttribute('aria-valuetext',value===null?'尚未评分':`${value} 星，满分 5 星`);
  renderRating($('author-star-preview'),value);renderRating($('author-preview-rating'),value);
  $('author-preview-place').textContent=m?.place||m?.title||'';
  $('author-preview-comment').textContent=$('author-comment').value||'尚未写下评价';
  $('author-preview-name').textContent=$('author-name').value?`— ${$('author-name').value}`:'未填写署名';
}
function selectEditor(id){
  editorId=id;const m=editorMemory();if(!m)return;
  $('author-place').value=id;$('author-name').value=m.review?.author??'谢老师';
  const value=m.review?.rating;$('author-unrated').checked=!validRating(value);$('author-rating').value=validRating(value)?String(value):'0';
  $('author-comment').value=m.review?.comment||'';previewAuthor();
}
function editorOptions(preferred){
  const options=editorItems.map(m=>{const option=text('option','',[m.place||m.title,m.date].filter(Boolean).join(' · '));option.value=m.id;return option;});
  $('author-place').replaceChildren(...options);selectEditor(editorItems.some(m=>m.id===preferred)?preferred:editorItems[0]?.id);
}
function openAuthor(id){
  if(!$('author-dialog').open&&editorLoaded)captureReview();
  if(!editorLoaded){editorItems=structuredClone(state.real);editorLoaded=true;}
  if(!editorItems.length){notify('还没有可编辑的真实记录。演示地点不会导出为真实故事。');return;}
  $('about-dialog').close();editorOptions(id||editorId);$('author-dialog').showModal();
  authorStatus('这里只编辑作者的评分与评价；照片、坐标及其他故事字段在导出时保留。');
  $('author-place').focus({preventScroll:true});
}
function closeAuthor(){captureReview();$('author-dialog').close();}
function localReviewDraft(){captureReview();return {schemaVersion:1,reviews:editorItems.map(m=>({id:m.id,review:m.review||null}))};}
function saveLocalDraft(){
  try{localStorage.setItem(draftKey,JSON.stringify(localReviewDraft()));authorStatus('草稿已保存在这个浏览器。尚未发布；另一个域名或设备不会自动同步。');}
  catch(e){authorStatus(`无法保存到本机：${e.message}。当前编辑仍在，可直接导出文件。`,true);}
}
function restoreLocalDraft(){
  try{
    const raw=localStorage.getItem(draftKey);if(!raw){authorStatus('这个浏览器还没有保存的草稿。');return;}
    const draft=JSON.parse(raw);if(draft?.schemaVersion!==1||!Array.isArray(draft.reviews))throw new Error('草稿格式不正确');
    const lookup=new Map(draft.reviews.map(m=>[m.id,m.review]));
    const proposed=editorItems.map(m=>lookup.has(m.id)?{...m,review:lookup.get(m.id)}:m);
    editorItems=validateMemories({schemaVersion:1,memories:proposed});selectEditor(editorId);authorStatus('已载入本机草稿。确认后导出文件，再提交发布。');
  }catch(e){authorStatus(`未能载入草稿：${e.message}`,true);}
}
function exportMemories(){
  try{
    captureReview();const clean=publishableMemories(editorItems);validateMemories(clean);
    const payload={...savedRoot,...clean};
    const blob=new Blob([JSON.stringify(payload,null,2)+'\n'],{type:'application/json;charset=utf-8'}),url=URL.createObjectURL(blob);
    const a=document.createElement('a');a.href=url;a.download='memories.json';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);
    authorStatus('文件已导出。请把它替换到 public/data/memories.json，再提交推送。当前线上评价尚未修改。');
  }catch(e){authorStatus(`导出失败：${e.message}`,true);}
}
$('author-form').addEventListener('submit',e=>e.preventDefault());
$('author-place').addEventListener('change',e=>{captureReview();selectEditor(e.target.value);});
for(const id of ['author-name','author-comment','author-rating','author-unrated'])$(id).addEventListener('input',previewAuthor);
$('rating-minus').addEventListener('click',()=>{$('author-rating').value=String(Math.max(0,Number($('author-rating').value)-.5));previewAuthor();});
$('rating-plus').addEventListener('click',()=>{$('author-rating').value=String(Math.min(5,Number($('author-rating').value)+.5));previewAuthor();});
$('story-author').addEventListener('click',()=>openAuthor(state.selected?.id));$('about-author').addEventListener('click',()=>openAuthor());
$('close-author').addEventListener('click',closeAuthor);$('author-dialog').addEventListener('cancel',e=>{e.preventDefault();closeAuthor();});
$('save-author-draft').addEventListener('click',saveLocalDraft);$('restore-author-draft').addEventListener('click',restoreLocalDraft);$('export-memories').addEventListener('click',exportMemories);
$('import-memories').addEventListener('change',async e=>{
  const file=e.target.files?.[0];if(!file)return;
  try{
    if(file.size>10*1024*1024)throw new Error('故事 JSON 文件应小于 10 MB，不要在其中嵌入图片。');
    const imported=JSON.parse(await file.text());const records=validateMemories(imported);
    if(!records.length)throw new Error('文件中没有真实记录；现有内容已保留。');
    captureReview();editorItems=mergeMemories(editorItems,records);savedRoot={...savedRoot,...imported};delete savedRoot.memories;
    editorOptions(editorId);authorStatus(`已合并导入 ${records.length} 条记录：相同 id 以导入文件为准，其余现有记录保留。尚未发布。`);
  }catch(error){authorStatus(`导入失败：${error.message}`,true);}finally{e.target.value='';}
});
