import {categoryOf,categoryIcon,ratingTone,CATEGORIES} from './place-style.v0.6.js';
...
function refreshCollection(){
  ...
  state.filtered=sortMemories(filterMemories(state.items,state.region,state.query).filter(m=>$('category-filter').value==='all'||categoryOf(m.category)===$('category-filter').value),$('rating-sort').value);
...
$('category-filter').addEventListener('change',refreshCollection);
