...
const fields={place:'place-name',placeEn:'place-en',region:'region',category:'category',locationLabel:'address',date:'trip-date',description:'description'};
...
function lock(value){busy=value;$('fields').disabled=value||!ready;for(const id of ['new-place','reload','place-select'])$(id).disabled=value||!ready;$('discard').disabled=value||!records.find(r=>r.id===selected)?.draft;}
...
function ratingUI(){const value=$('unrated').checked?null:Number($('rating').value);$('rating-value').textContent=value===null?'未评分':`${value.toFixed(1)} / 5 ★`;$('rating').disabled=$('unrated').checked;}
...
function select(id,newPlace=null){
  ...
  if(!current)return;photos=structuredClone(photosOf(current));
  for(const [key,field]of Object.entries(fields))$(field).value=current[key]|| (key==='region'?'england':'');
  $('longitude').value=current.coordinates?.[0]??'';$('latitude').value=current.coordinates?.[1]??'';
  $('review-author').value=current.review?.author||'';$('comment').value=current.review?.comment||'';
  $('unrated').checked=current.review?.rating==null;$('rating').value=current.review?.rating??5;ratingUI();
  dirty=false;options();renderPhotos();lock(false);
...
async function load(){
  ...
  try{
    ...
    const [seed,saved,cloud]=await Promise.all([api('/data/fountains-abbey.v0.3.json'),api('/data/memories.json'),api('/author/api/places')]);
...
function createPlace(){select('place-'+crypto.randomUUID(),{place:'',title:'',region:'england',coordinates:null,photos:[],review:{author:$('review-author').value||'',rating:null,comment:''}});dirty=true;$('place-name').focus();message('新地点已准备好：填写名称，在地图上点击位置，再添加照片与评价。');}
...
$('place-form').addEventListener('input',()=>{dirty=true;ratingUI();drawPins();});
